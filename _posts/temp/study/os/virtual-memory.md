...
- 실제 사용하는 메모리는 작지만, 큰 공간을 제공하는 것처럼 보이게 한다. 
- 프로세스는 가상 주소를 사용하고, 실제 해당 주소가 참조될 때만 물리 주소로 변환한다. 
...
- 비트가 유효(1): 해당 페이지가 메모리에 존재
- 비트가 무효(0): 해당 페이지가 메모리에 없음 
