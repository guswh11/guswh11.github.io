## 파일의 개념
파일 시스템은 다음과 같이 두 부분으로 구성된다. 
- 자료를 저장하는 **파일**들의 모음
- 시스템 내의 모든 파일을 정리하고 그에 관한 정보를 제공하는 **디렉터리 구조**

파일: 저장 장치의 물리적 특성을 추상화하여 정의된 논리적 저장 단위
- 저장된 정보에 대한 일관된 논리적 관점을 제공
- 보조 저장 장치에 기록된 관련 정보의 집합
- 일반적으로 프로그램과 자료를 나타냄

### 파일 연산(File Operations)
아래는 파일 조작에 필요한 최소 연산들이다. 
- 파일 생성
...
- 파일 안에서의 위치 재설정(reposition, 파일 탐색)
    - 디렉터리에서 적합한 항목을 탐색하고 현재 파일 위치를 주어진 값으로 설정
- 파일 삭제
- 파일 절단: 파일의 내용은 지우고 그 파일의 속성은 그대로 남기기를 원할 때 사용함
    - 파일의 길이를 제외한 모든 속성은 그대로 유지됨
    - 파일의 길이가 0으로 재설정되며 파일이 가지고 있던 공간은 해제됨

운영체제는 일반적으로 다음과 같은 두 단계 내부 테이블을 사용한다.

**열린 파일 테이블**(open-file table): 모든 열린 파일에 대한 정보를 유지
- 프로세스에 독립적인 정보를 가짐
    - 디스크 상의 파일 위치, 접근 날짜, 파일 크기 등
- 각 파일마다 연 프로세스 수를 가리키는 **열린 계수**(open count)를 기록함

**프로세스별 테이블**: 각 프로세스가 연 모든 파일들을 기록
- 프로세스가 파일을 어떻게 사용하는지 저장됨
    - 각 파일에 대한 현재 파일 포인터, 다음에 수행될 시스템 호출이 영향을 미칠 파일의 위치, 파일에 대한 접근 권리, 회계 정보 등
- 각 항목은 범 시스템 열린 파일 테이블을 가리킴 
...
### 파일 내부 구조(Internal File Structure)
...
![sequential access](sequential-access.png)
...
**직접 접근(Direct Access)**<br>
파일을 구성하는 레코드를 임의의 물리적 저장공간에 기록하는 것을 의미한다.<br>
...
![sequential/direct access](sequential-direct-access.jpg)
...
**색인 순차 파일(Indexed Sequential File)**<br>
순차 파일과 직접 파일에서 지원하는 편성 방법이 결합된 형태이다.
색인(인덱스)을 이용한 순차적인 접근 방법을 제공하여 색인 순차 접근 방식(ISAM; Index Sequential Access Method)이라고도 한다.
순차 처리와 랜덤 처리가 모두 가능하도록 레코드들을 키 값 순으로 정렬(sort)시켜 기록하고, 레코드의 키 항목만을 모은 색인을 구성하여 편상하는 방식이다. 
...
### 일관성 의미(Consistency Semantics)
일관성 의미는 여러 사용자가 동시에 공유 파일에 접근하는 방법을 명시한다. 예를 들어, 한 사용자가 데이터를 수정했을 때 그것이 언제 다른 유저들에게 공유될 것인지 등에 대한 것들을 지정한다. 일관성 의미는 파일 시스템의 코드로서 구현된다. 

일관성 의미는 프로세스 동기화 알고리즘과 관련이 있지만, 프로세스 동기화 알고리즘은 파일 입출력의 경우에는 구현되지 않는다는 차이점이 있다. 

특정 파일에 대한 한 사용자의 접근은 항상 열기(open)와 닫기(close) 연산에 둘러싸여 있다고 가정한다. 열기와 닫기 연산 사이의 접근은 하나의 **파일 세션**을 생성한다. 

일관성 의미의 예시에는 다음과 같은 종류가 있다. 

![consistency semantics](consistency-semantics-example.png)

**유닉스의 의미(UNIX Semantics)**
- 열린 파일에 대한 사용자의 쓰기는 동일 파일을 연 다른 사용자들에게 즉시 보인다. 
- 사용자들이 파일 내의 현재 위치 포인터를 공유하는 공유 모드가 있다. 한 사용자가 포인터를 전진시키면 공유하고 있는 모든 사용자들에게 영향을 미친다. 파일에 대한 모든 접근은 서로 번갈아 가면서 실행된다. 

**세션 의미(Session Semantics)**
- 열린 파일에 대한 한 사용자의 쓰기는 동시에 동일 파일을 연 다른 사용자들에게 바로 보이지 않는다. 
- 파일이 닫히면, 파일에 대한 변경들은 나중에 시작되는 세션에서만 보인다. (이미 열린 파일의 인스턴스들은 변경을 반영하지 않는다.)

**불변 공유 파일의 의미(Immutable-Shared-Files Semantics)**
- 파일이 공유된다고 선언되면, 파일 내용은 더 이상 변경될 수 없다. (읽기만 가능)
- 파일 이름은 재사용될 수 없다. 
