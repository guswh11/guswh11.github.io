...
하드디스크에서 ECC를 지원하지 않기 때문에 ECC를 별도의 드라이브에 저장하는 방식이다.
패리티 정보를 하나의 하드 드라이브에 저정하는 RAID 4가 나오면서 현재는 더 이상 사용되지 않는 구성이다. 
...
**RAID Level 3**
메모리 시스템과는 달리 한 섹터가 정확히 읽혔는지를 디스크 컨트롤러가 탐지할 수 있다는 사실을 이용해 RAID 2의 기능을 향상시킨 것이다. 패리티 비트는 오류 정정 뿐만 아니라 오류 탐지에도 이용할 수 있다.<br>

바이트 단위 스트라이핑을 사용하며, 하나의 드라이브는 패리티 정보를 저장하는 데 사용된다. I/O 작업이 동시에 모든 드라이브를 처리하므로 I/O 작업을 중첩할 수 없다. 따라서 수행시간이 긴 프로그램 위주의 단일 사용자 시스템에 적합하다. 
...
**RAID Level 4**
블록 단위 스트라이핑을 사용하며, RAID 3과 마찬가지로 별도의 디스크에 패리티 정보를 저장한다.
하나의 블록 읽기는 하나의 디스크에만 접근하므로 나머지 디스크들은 다른 요구를 처리할 수 있다. 모든 디스크가 병렬적으로 접근될 수 있기 때문에 대규모 읽기와 쓰기에 대한 전송률이 높다. 
...
**RAID Level 5**
RAID 3,4에서 별도의 패리티 정보 디스크를 사용함으로써 발생하는 문제점을 보완해 패리티 비트들을 모든 디스크에 분산시켜 저장하는 방법이다. 
하나의 디스크가 고장 나더라도 남은 디스크를 통해 데이터를 복구할 수 있다. 
...
**RAID Level 6**
RAID 5보다 데이터의 안전성을 고려한 구조로, 패리티 비트 대신 Reed-Solomon Codes 같은 에러 교정 코드를 데이터 4비트마다 2비트씩 중복해 저장한다. P+Q 중복 기법이라고 불리며, 추가적인 중복 정보를 저장해 2개의 디스크에 문제가 생겨도 복구할 수 있도록 한다. 
...
**RAID Level 1+0**
디스크를 미러링 한 후, 그 쌍을 스트라이프하는 방식이다. 하나의 디스크에서 오류 발생 시 해당 디스크는 접근 불가능하지만 그것이 미러링 된 디스크와 다른 모든 디스크는 사용 가능하다. 
